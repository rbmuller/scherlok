...
- [ ] Create repo `github.com/rbmuller/scherlok`
- [ ] Python project structure (src/scherlok/, tests/, pyproject.toml)
- [ ] CLI skeleton with Click or Typer (`scherlok connect`, `scherlok investigate`, `scherlok watch`)
- [ ] CI/CD with GitHub Actions (pytest + linting)
- [ ] Basic README with install instructions

### Week 3-4: Postgres profiler
- [ ] `scherlok connect postgres://...` — validate connection, discover tables
...
- [ ] Store profiles in local SQLite (`.scherlok/profiles.db`)
- [ ] `scherlok report` — print profile summary to terminal

**Deliverable:** `pip install scherlok` works. Can profile a Postgres database.
...
- [ ] `scherlok watch` — compare current state vs last profile:
  - Volume: row count deviation from historical average (z-score)
  - Freshness: table not updated within expected cadence
  - Nullability: NULL rate changed significantly
  - Distribution: mean/stddev shifted beyond threshold
  - Cardinality: number of distinct values changed unexpectedly
  - Schema: column added, removed, or type changed
- [ ] Severity scoring: INFO / WARNING / CRITICAL
- [ ] Auto-calibrated thresholds (no manual config needed)

### Week 7-8: Alerting
- [ ] `scherlok watch --slack https://hooks.slack.com/...` — Slack notifications
- [ ] `scherlok watch --exit-code` — exit 1 on CRITICAL (for CI/CD)
- [ ] Alert format: what changed, when, how much, suggested action
